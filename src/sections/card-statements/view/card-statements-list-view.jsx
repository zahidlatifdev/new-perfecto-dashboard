...
export function CardStatementsListView() {
    ...
    const fetchCardStatements = async () => {
        try {
            ...
            // Make single API call with proper error handling
            ...
            const cardStatements = response.data.data?.statements || [];

            // Sort statements by statement period start date (oldest first)
            const sortedStatements = sortStatementsByPeriod(cardStatements);
            setStatements(sortedStatements);
        } catch (err) {
            console.error('Failed to fetch card statements:', err);
            setError('Failed to load card statements. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleViewTransactions = (statement) => {
        router.push(`/dashboard/card-statements/${statement._id}/transactions?statementId=${statement._id}&type=card`);
    };
    ...
    const handleUploadNew = () => {
        router.push('/dashboard/card-statements/upload');
    };
    ...
    const renderEmptyState = () => (
        ...
        >
            ...
            <Typography variant="body1" sx={{ mb: 4, color: 'text.secondary', maxWidth: 400, mx: 'auto' }}>
                Upload your first credit card statement to start tracking transactions and analyzing your spending patterns automatically.
            </Typography>
            <Button
                variant="contained"
                size="large"
        ...
        return (
            <DashboardContent>
                <Alert severity="warning" sx={{ mb: 3 }}>
                    ...
                    Please select a company to view credit card statements.
                </Alert>
            </DashboardContent>
        );
    }
                                    ...
                                    <Stack spacing={2}>
                                        ...
                                        {statement.creditCard && (
                                            ...
                                            >
                                                <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 0.5 }}>
                                                    <Iconify
                                                        icon={getCardTypeIcon(statement.creditCard.cardType)}
                                                        sx={{ fontSize: 20 }}
                                                    />
                                                    <Typography variant="subtitle2" color="secondary.main">
                                                        {statement.creditCard.issuerBank}
                                                    </Typography>
                                                </Stack>
                                                <Typography variant="body2" color="secondary.dark">
                                                    {statement.creditCard.cardName}
                                                </Typography>
                                                {statement.creditCard.lastFourDigits && (
                                        ...
                                        <Stack spacing={1}>
                                            ...
                                            {statement.accountDetails && (
                                                ...
                                                >
                                                    <Stack spacing={0.5}>
                                                        {statement.accountDetails.totalBalance !== undefined && (
                                                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                                                <Typography variant="caption" color="text.secondary">
                                                                    Total Balance:
                                                                </Typography>
                                                                <Typography variant="caption" fontWeight="medium" color="text.primary">
                                                                    ${statement.accountDetails.totalBalance?.toFixed(2) || '0.00'}
                                                                </Typography>
                                                            </Box>
                                                        )}
                                                        {statement.accountDetails.availableCredit !== undefined && (
                                                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                                                <Typography variant="caption" color="text.secondary">
                                                                    Available Credit:
                                                                </Typography>
                                                                <Typography variant="caption" fontWeight="medium" color="text.primary">
                                                                    ${statement.accountDetails.availableCredit?.toFixed(2) || '0.00'}
                                                                </Typography>
                                                            </Box>
                                                        )}
                                                        {statement.accountDetails.creditLimit !== undefined && (
                                                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                                                <Typography variant="caption" color="text.secondary">
                                                                    Credit Limit:
                                                                </Typography>
                                                                <Typography variant="caption" fontWeight="medium" color="text.primary">
                                                                    ${statement.accountDetails.creditLimit?.toFixed(2) || '0.00'}
                                                                </Typography>
                                                            </Box>
                                                        )}
                                                    </Stack>
                ...
                <DialogContent>
                    {viewDialog.statement?.filePath ? (
                        ...
                        <Alert severity="info">
                            ...
                            File preview is not available for this statement. You can download it to view the contents.
                        </Alert>
                    )}
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setViewDialog({ open: false, statement: null })}>
                        Close
                    </Button>
                    {viewDialog.statement?.filePath && (
                        <Button
            ...
            >
                ...
                <DialogContent>
                    <Typography variant="body1" sx={{ mb: 2 }}>
                        Are you sure you want to delete this credit card statement?
                    </Typography>
